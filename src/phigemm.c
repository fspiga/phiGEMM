		...
		if ( is_splitA ) {
			...
			printf ("[%s:%s - GPU %d] %d (%d %d, %5.4f) %d %d ~ H2D:%9.6fs (%6.4fGB/s) MKL:%9.6fs (%5.4fGflops) CUBLAS: %9.6fs (%7.4fGflops) D2H:%9.6fs (%6.4fGb/s) ~ BALANCE: %9.6fs ~ Total: %9.6fs (%7.4fGflops)\n",
					...
					*k,
					...
					time_dgemm_cuda,
					1.e-6 * PHIGEMM_FLOPS( (double)m_gpu[iDevice], (double)(*n), (double)(*k) )/(time_dgemm_cuda*1000),
					time_mem_d2h,
			...
			printf ("[%s:%s - GPU %d] %d %d (%d %d, %5.4f) %d ~ H2D:%9.6fs (%6.4fGB/s) MKL:%9.6fs (%5.4fGflops) CUBLAS: %9.6fs (%7.4fGflops) D2H:%9.6fs (%6.4fGb/s) ~ BALANCE: %9.6fs~ Total: %9.6fs (%7.4fGflops)\n",
					...
					*k,
					...
					time_dgemm_cuda,
					1.e-6 * PHIGEMM_FLOPS( (double)(*m), (double)n_gpu[iDevice], (double)(*k) )/(time_dgemm_cuda*1000),
					time_mem_d2h,
					m_gpu[iDevice]*n_gpu[iDevice]/time_mem_d2h/(1024*1024*1024/sizeof(XTYPE)),
					unbalance,
					time_total,
